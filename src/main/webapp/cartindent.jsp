    ...
    <form id="frm" action="<%=path%>/is/addorder" method="post">
        ...
        <div class="Consigneew boxS w1190">
            <div class="Consignee clearfix">
                ...
                <ul  id="idorder" class="address clearfix">
                    <c:forEach items="${listOrder}" var="v">
                        <c:choose>
                            <c:when test="${v.recipientsChecked==1}">
                                <li class="checkbox checked">
                                    <p class="user">${v.recipientsName}</p>
                                    <p class="site">${v.recipientsAddress}</p>
                                    <p class="phonenum">${v.recipientsPhone}
                                        <input class="disNone" name="orderaddressid" type="radio"  checked="checked" value="${v.recipientsId}" >
                                    </p>
                                </li>
                            </c:when>
                            <c:otherwise>
                                <li class="checkbox">
                                    ...
                                    <p class="site">${v.recipientsAddress}</p>
                                    <p class="phonenum">${v.recipientsPhone}
                                        <input class="disNone"  name="orderaddressid" type="radio" value="${v.recipientsId}" >
                                    </p>
                                </li>
                            </c:otherwise>
                        </c:choose>
                    ...
                    <c:forEach items="${list }" var="v">
                        <div class="prouduct clearfix">
                            <ul class="blur clearfix">
                                <li class="first">
                                    <p>
                                        <input class="disNone" name="shopid" type="checkbox" checked="checked"  value="${v.item_id}"/>
                                        ${v.goods_name }</p>
                                </li>
                                <li>${v.item_num }</li>
                                <li>￥${v.goods_price }</li>
                                <li>200g</li>
                                <li>￥114.0</li>
                                <li class="allpri">￥${v.item_num*v.goods_price }</li>
                            </ul>
                        </div>
                ...
                <div class="blurFooterw clearfix">
                    <div class="blurFooter">
                        ...
                        <p class="submit clearfix">
                            <span>返回购物车</span>
                            <span>实付总额： <em class="totalP">￥116.00</em></span>
                            <a href="javascript:void(0)" onclick="document.getElementById('frm').submit()">提交订单</a>
                        </p>
                        <p class="protocol clearfix">
                            <span class="checkbox checked"></span>
                            <span>本人同意并接受《淘宝海购服务协议》与《进口个人委托申报委托》</span>
                        </p>
                    </div>
                </div>
            </div>
...
<div class="mode">
    <div class="modeArea">
        ...
        <p class="Rpeople">
            ...
            <input id="sname" class="w170" type="text" placeholder="限20个字">
        </p>
        <p class="Receiving">
            <strong>
                <em>*</em>
        ...
        <p class="Detailed">
            ...
            <input id="recipientsAddress" class="w370" type="text" placeholder="无需重复填写省市区，小于50个字">
        </p>
        <p class="IDcard">
            <strong>
                <span>身份证号</span>
            ...
            <em>?
                <u>
                    <b>
                        根据海关要求，购买跨境商品需实名认证，进行个人物品入境申报。因各保税区政策不同，郑州保税区需您提供收获人身份信息进行认证。其他报税区将通过支付平台实名信息进行入境申报，您可在支付平台进行认证（如已认证直接付款即可）。淘宝会对您的身份信息进行加密保管，绝不泄露！
                    </b>
                    有任何身份认证的问题请咨询我们<br>
                    400-115-8086<br>
                    09:00-24:00<br>
        ...
        <p class="phonenum">
            ...
            <input id="recipientsPhone" class="w170" type="text" placeholder="请输入正确的手机号">
        </p>
        <p class="Tel">
            <strong>
                <span>固定电话</span>
        ...
        <p class="sure">
            <strong></strong>
            <button type="button" onclick="subadd()">确定</button>
            <a id="closevw" href="#">取消</a>
            ...
            <script type="text/javascript">
                function subadd() {
                    ...
                    var recipientsPhone = $("#recipientsPhone").val();

                    //以ajax的方式提交到增加用户地址的controller
                    $.post("/abcd/is/addorderrecipients", {
                        recipientsName : sname,
                        recipientsAddress : recipientsAddress,
                        recipientsPhone : recipientsPhone
                    }, function(data) {
                        if(data==1){//增加成功
                            var s='<li class="checkbox">';
                            s+='<p class="user">'+sname+'</p>';
                            s+='<p class="site">'+recipientsAddress+'</p>';
                            s+='<p class="phonenum">'+recipientsPhone+'</p>';
                            s+='<input name="orderaddressid" type="radio" checked="checked" value="${v.recipientsId}">';
                            s+='</li>';
                            $("#idorder").append(s);
                            $("#closevw").click();
                        }
...
<script>
    ...
    window.onload = function (ev) {
        ...
        var totalPL = document.getElementsByClassName("totalP");
        var totalP = 0;
        for(var i = 0, len = allpri.length; i < len; i++){
            totalP += parseFloat(allpri[i].innerHTML.replace("￥", ""));
        }
        for (var i = 0, len = totalPL.length; i <  len; i++) {
            totalPL[i].innerHTML = "￥" + totalP;
        }
